...
export type Pack = { id: string; source: string; version: string; revision: string; skills: string[]; description: string };
export type Catalog = { schemaVersion: 2; packs: Pack[] };
export type Config = { schemaVersion: 1; harness: Harness; directory: string };
export function parseCatalog(value: unknown): Catalog {
  const c = value as Catalog;
  if (!c || c.schemaVersion !== 2 || !Array.isArray(c.packs) || !c.packs.length) throw new Error("Catalog requires the current Wizzy CLI. Pull this repository and reinstall dependencies.");
  const ids = new Set<string>();
  const names = new Set<string>();
  for (const p of c.packs) {
    if (!p || typeof p.id !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(p.id) || p.source !== `galleonlabs/${p.id}` || typeof p.description !== "string" || ids.has(p.id)) throw new Error("Invalid or duplicate catalog pack; only explicit galleonlabs repositories are allowed");
...
export async function fetchCatalog(fetcher: (url: string, options?: RequestInit) => Promise<Response> = fetch): Promise<Catalog> {
  const response = await fetcher("https://raw.githubusercontent.com/galleonlabs/WizzyBot/main/catalog/skills.json", { signal: AbortSignal.timeout(20_000) });
...
export function catalogChanges(current: Catalog, previous?: Catalog): string[] {
  ...
  for (const pack of current.packs) {
    ...
    if (!old || old.revision !== pack.revision || old.version !== pack.version || JSON.stringify(old.skills) !== JSON.stringify(pack.skills)) changes.push(`${pack.id}: ${old?.version ?? "unversioned or absent"} -> ${pack.version} (${pack.revision.slice(0, 7)})`);
  }
